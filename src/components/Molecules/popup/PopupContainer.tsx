...
const PopupContainer = ({ isShown, title, onClose, className, children, ...rest }: LayoutProps ) => (
    <AnimatePresence exitBeforeEnter={true}>
      {isShown && 
        ...
        >
          <div className="fixed w-full h-full bg-[#00000045] hover:cursor-pointer">&nbsp;</div>
          <motion.div 
            initial={'hidden'}
            animate={'visible'}
            exit={'hidden'}
            variants={popupContainerAnimation}
            className={'fixed z-[150] bg-white w-1/2 min-h-[300px] ' + (className && className)}
          >
