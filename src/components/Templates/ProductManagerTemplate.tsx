...
const ProductManagerTemplate = () => {
  ...
  return (
   <>
     <PopupContainer isShown={isShownPopup} title={popupTitle} onClose={handleClickEvent.registProduct} className='w-[600px]'>
      ...
      />
      <div className='mt-8'>
        <Button variant="contained" color="primary" >{popupTitle}</Button>
      </div>
     </PopupContainer>
     <div className="btnGroup pb-8 ">
       <ButtonGroup aria-label="contained primary button group" className='mt-8'>
           <Button variant="outlined" onClick={handleClickEvent.registProduct}>상품 등록</Button>
           <Button variant="outlined" color="primary" onClick={handleClickEvent.modifyProduct}>상품 수정</Button>
           <Button variant="outlined" color="secondary">상품 삭제</Button>
       </ButtonGroup>
     </div>
              ...
              {productListMocks.map((item: productListType, index: number) => {
                return (
                  <tr key={item.productIndex.toString()}>
                    ...
                    <td>{item.productOption ? item.productOption : '미등록'}</td>
