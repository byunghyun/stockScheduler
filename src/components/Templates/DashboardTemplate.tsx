          ...
          <ul className='flex'>
              {Object.keys(currentStatus).map((items, index) => {
                  ...
                  switch (items) {
                      case 'warehousing':
                          resultData.title = '입고';
                          resultData.percenter = (currentStatus[items].today / currentStatus[items].yesterday) * 100;
                          resultData.sign = !(currentStatus[items].today / currentStatus[items].yesterday) ? '' : (currentStatus[items].today / currentStatus[items].yesterday) >= 1 ? '+' : '-';
                          break;
                      case 'shipments':
                          resultData.title = '출고';
                          resultData.percenter = (currentStatus[items].today / currentStatus[items].yesterday) * 100;
                  ...
                  return (
                      <li key={index.toString()} className={'flex flex-col justify-between flex-1 h-[130px] text-[13px] bg-white p-4 shadow-md rounded-lg' + (index !== Object.keys(currentStatus).length - 1 ? ' mr-4': '')}>
                        <p>{resultData.title}</p>
                        <div>
                            <p className='text-[30px] font-semibold mb-0'>{currentStatus[items].today}</p>
                              <p className={resultData.sign === '+' ? `text-[#009688]` : resultData.sign === '-' ? `text-[#db5050]` : `text-[#525252]`}>{resultData.sign}{Math.round(resultData.percenter) ? `${Math.round(resultData.percenter)}%` : '0%'} 전일대비</p>
                        </div>
                    </li>
                  );
              })}
